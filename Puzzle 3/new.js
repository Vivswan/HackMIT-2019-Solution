let solution = ""
const clues = [
    {
        hint: 'places',
...
a = document.querySelectorAll("#root > div > div:nth-child(2) > div > div.gridSection > table > tbody input")
for(i of "abcdefghijklmnopqrstuvwxyz"){
    let x = '#'+(function lol(m,s,c){return s[m.floor(m.random() * s.length)] +(c && lol(m,s,c-1));})(Math,'0123456789ABCDEF',4)
    for (j of a){
        if (j.value === i)
            j.style.background = x;
    }
}
